...
# yearly temperature anomalies (reference period 1981-2010: https://aemetblog.es/2024/07/24/preguntas-y-respuestas-sobre-las-olas-de-calor/)
ref_tmean <- ace_data %>%
...
ggsave("3_Graphs/Max temp calendar.png",
       plot = last_plot())
...
ace_data %>%
  ...
  add_source(source = html("<b>Table 1.</b> Number of days with high temperatures.<br>
                           Data: Aemet.<br>Made by Oliver Q.R."),
             font_size = 14,
             font_style = "italic",
             font_color = "grey50",
             align = "left",
...
ggsave("3_Graphs/Min temp calendar.png",
       plot = last_plot())
...
ace_data %>%
  ...
  add_source(source = html("<b>Table 2.</b> Number of days according to the min. temperature.<br>
                           Data: Aemet.<br>Made by Oliver Q.R."),
             font_size = 14,
             font_style = "italic",
             font_color = "grey50",
...
ggsave("3_Graphs/Monthly mean temp anomaly.png",
       plot = last_plot())
...
ace_data %>%
  ...
  add_source(source = html("<b>Table 3.</b> Monthly average temperature differences.<br>
                           Data: Aemet.<br>Made by Oliver Q.R."),
             font_size = 14,
             font_style = "italic",
             font_color = "grey50",
...
ace_data %>%
  ...
  geom_rect(aes(xmin = -200, xmax = 0, ymin = -1.5, ymax = 0),
  ...
  geom_rect(aes(xmin = -200, xmax = 0, ymin = 0, ymax = 2.5),
            fill = "#fdae61", alpha = 0.1) +
  annotate(geom = "text", label = "Colder\nDrier", x = -195, y = -1.25, colour = "white",
  ...
  annotate(geom = "text", label = "Warmer\nWetter", x = 195, y = 2.25, colour = "white",
           hjust = "right", fontface = "bold") +
  annotate(geom = "text", label = "Warmer\nDrier", x = -195, y = 2.25, colour = "white",
           hjust = "left", fontface = "bold") +
...
ace_data %>%
  ...
  labs(title = "Annual mean temperature at Lanzarote Airport (Jan to Sept)",
       subtitle = "Reference period: 1973-2004",
       x = "Year",
       y = "Mean temperature (°C)",
       caption = "Data: Aemet.\nMade by Oliver Q.R.") +
  theme_bw(base_family = "Arial") +
...
ace_data %>%
  ...
  geom_label_repel(data = ~.x %>% filter(prec_an == min(prec_an)),
                   aes(x = year, y = prec_an, label = paste0(round(prec_an, 2), "%")),
                   size = 3.5, nudge_x = -0.5, nudge_y = -25, fill = "white",
                   colour = "#67001f", alpha = 0.5) +
  geom_label_repel(data = ~.x %>% filter(prec_an == max(prec_an)),
                   aes(x = year, y = prec_an, 
                       label = paste0("+", round(prec_an, 2), "%")),
                   size = 3.5, nudge_x = 1, nudge_y = 1, fill = "white",
                   colour = "darkblue", alpha = 0.5) +
  geom_label_repel(data = ~.x %>% filter(year == 2024),
                   aes(x = year, y = prec_an, label = paste0(round(prec_an, 2), "%")),
                   size = 3.5, nudge_x = -0.5, nudge_y = -25, fill = "white",
                   colour = "#67001f", fontface = "bold") +
  scale_x_continuous(breaks = seq(1975, 2020, 5)) +
  ...
  scale_fill_gradient2(low = "#9e0142", mid = "#f7f7f7", high = "#5e4fa2",
                       midpoint = 0,
                       limits = c(-200, 200)) +
  labs(title = "Precipitation anomalies at Lanzarote Airport from Jan to Sept",
       subtitle = "Deviations from the annual mean precipitation for the period 1973-2000",
       x = "Year",
       y = "Precipitation anomaly (%)",
       caption = "Data: Aemet.\nMade by Oliver Q.R.",
...
ggsave("3_Graphs/Prec anomalies jan-sept.png",
       plot = last_plot())
